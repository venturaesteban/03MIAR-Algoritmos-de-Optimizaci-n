# Algoritmos de Optimización

Este repositorio es parte del curso de Master de AI en VIU, específicamente en la asignatura 'Algoritmos de Optimización'. Aquí se trabajará principalmente con **Python** y se usarán **notebooks de Google Colab** para facilitar el aprendizaje y la experimentación.

## Estructura del repositorio

El repositorio se sugiere que tenga la siguiente estructura:

```
03MIAR-Algoritmos-de-Optimizaci-n/
│
├── notebooks/ # Actividades Guiadas, Trabajos y Proyectos
├── scripts/   # Scripts de código python
├── data/      # Ficheros de datos (doc, pdf, csv...) con los que trabajar
└── README.md
```

## Cómo ejecutar los notebooks

Para ejecutar los notebooks, puedes clonarlo en tu máquina local y usar Jupyter Lab para trabajar con ellos, y/o abrir los archivos `.ipynb` en Google Colab.

## Consejos sobre el entorno

Si ejecutas los notebooks en local, asegúrate de tener un entorno de Python configurado con las librerías requeridas. Puedes crear un entorno virtual usando `venv` o `conda`, lo que facilitará la gestión de dependencias.
